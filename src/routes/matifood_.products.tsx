...
import { getLocations, type Location } from "@/lib/locations.functions";
...
export const Route = createFileRoute("/matifood_/products")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse MatiFood's live product catalogue and place an order for fresh mango and more.",
      },
    ],
  }),
    ...
    <>
      <Section>
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-4 text-muted-foreground">Pulled live from the nisilagro.com product database.</p>
          <button
            onClick={() => refetch()}
            disabled={isFetching}
        ...
        <div className="mt-12">
          {isLoading ? (
            ...
            <p className="text-center text-destructive">{data?.error ?? "Failed to load products."}</p>
                ...
                <tbody>
                  {data?.products.map((p) => (
                    <tr key={p.id} className="border-t border-border">
                      ...
                      <td className="px-6 py-4 text-sm text-muted-foreground">{p.created_at ? new Date(p.created_at).toLocaleDateString() : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function OrderForm() {
  ...
  const { data: locations } = useQuery({ queryKey: ["matifood-locations"], queryFn: () => fetchLocations() });
  ...
  const [status, setStatus] = useState<{ kind: "idle" } | { kind: "pending" } | { kind: "ok"; id?: number } | { kind: "error"; msg: string }>({ kind: "idle" });
  ...
  return (
    <section className="bg-secondary/60">
      <div className="mx-auto max-w-3xl px-4 py-20 sm:px-6 lg:px-8">
        ...
        <form onSubmit={onSubmit} className="mt-10 space-y-8 rounded-2xl border border-border bg-card p-8 shadow-[var(--shadow-soft)]">
          {/* Customer */}
          <fieldset className="space-y-4">
            <legend className="font-serif text-lg font-bold">👤 Your Details</legend>
            <Field label="Full name *">
              <input required maxLength={120} value={form.customer_name} onChange={(e) => set("customer_name", e.target.value)} className={inputCls} />
            </Field>
            <Field label="Phone *">
              <input required maxLength={40} value={form.phone} onChange={(e) => set("phone", e.target.value)} className={inputCls} />
            </Field>
            <Field label="Email (optional)">
              <input type="email" maxLength={200} value={form.email} onChange={(e) => set("email", e.target.value)} className={inputCls} />
            </Field>
          </fieldset>
          ...
          <fieldset className="space-y-3">
            ...
            <Field label="Delivery address *">
              <textarea required rows={3} maxLength={500} value={form.delivery_address} onChange={(e) => set("delivery_address", e.target.value)} className={inputCls} />
            </Field>
          </fieldset>
          ...
          <fieldset className="space-y-3">
            ...
            {MENU.map((m) => (
              <label key={m.label} className="flex cursor-pointer items-center justify-between gap-3 rounded-md border border-border p-3 hover:bg-secondary/60">
                <span className="flex items-center gap-3">
                  <input type="radio" name="product" value={m.label} checked={form.product === m.label} onChange={() => set("product", m.label)} />
                  <span>{m.label}</span>
                </span>
            ...
            <Field label="Quantity (boxes) *">
              <input type="number" min={1} max={50} required value={form.quantity} onChange={(e) => set("quantity", Math.max(1, Math.min(50, Number(e.target.value) || 1)))} className={inputCls} />
            </Field>
            <div className="rounded-md bg-secondary/60 p-3 text-right font-semibold">Total: ৳ {total.toLocaleString()}</div>
          </fieldset>
          ...
          <fieldset className="space-y-3">
            ...
            {(["Cash On Delivery", "bKash", "Other"] as const).map((p) => (
              <label key={p} className="flex cursor-pointer items-center gap-3 rounded-md border border-border p-3 hover:bg-secondary/60">
                <input type="radio" name="payment" value={p} checked={form.payment_method === p} onChange={() => set("payment_method", p)} />
                <span>{p === "Cash On Delivery" ? "Cash On Delivery | ক্যাশ অন ডেলিভারি" : p === "bKash" ? "bKash | বিকাশ" : "Other"}</span>
              </label>
            ))}
            {form.payment_method === "Other" && (
              <Field label="Payment note">
                <input maxLength={200} value={form.payment_note} onChange={(e) => set("payment_note", e.target.value)} className={inputCls} />
              </Field>
            )}
          </fieldset>

          <button type="submit" disabled={status.kind === "pending"} className="w-full rounded-md bg-primary px-6 py-3 text-base font-semibold text-primary-foreground transition hover:bg-primary-glow disabled:opacity-50">
            {status.kind === "pending" ? "Submitting…" : "Place Order"}
          </button>

          {status.kind === "ok" && <p className="text-center text-sm font-medium text-primary">✅ Order received{status.id ? ` (ref #${status.id})` : ""}. We'll contact you shortly.</p>}
